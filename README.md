# ЛКМ СНАБ — сайт (светлый редизайн)
...
- `api/` — serverless-функции Vercel (приём заявок, отправка письма менеджерам)
...
| `RESEND_API_KEY` | ключ [resend.com](https://resend.com), которым сайт отправляет письма с заявками |
| `ORDERS_EMAIL_TO` | почта менеджеров, куда падают заявки; несколько — через запятую |
| `ORDERS_EMAIL_FROM` | необязательно. Адрес отправителя, например `ЛКМ СНАБ <заказы@вашдомен.ру>` после подтверждения домена в Resend. Пока не задан — письма шлёт `onboarding@resend.dev` |
